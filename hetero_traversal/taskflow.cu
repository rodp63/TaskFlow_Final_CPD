  ...
  for(const auto& v : g.nodes) {
    ...
    if(v.g == -1) {
      ...
      tasks[v.v] = taskflow.emplace_on([&](tf::cudaFlow& cf){
        ++counter;
        auto sgx = cf.zero(gx, N).name("sgx");
        auto sgy = cf.zero(gy, N).name("sgy");
        auto sgz = cf.zero(gz, N).name("sgz");
        auto h2d_gx = cf.copy(gx, cx, N).name("h2d_gx");
        auto h2d_gy = cf.copy(gy, cy, N).name("h2d_gy");
        auto h2d_gz = cf.copy(gz, cz, N).name("h2d_gz");
        auto kernel = cf.kernel((N+255)/256, 256, 0, add<int>, gx, gy, gz, N).name("kernel");
        auto d2h_gx = cf.copy(cx, gx, N).name("d2h_gx");
        auto d2h_gy = cf.copy(cy, gy, N).name("d2h_gy");
        auto d2h_gz = cf.copy(cz, gz, N).name("d2h_gz");
        sgx.precede(h2d_gx);
        sgy.precede(h2d_gy);
        sgz.precede(h2d_gz);
